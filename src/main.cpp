...
void setup() {
  ...
  UBYTE *BlackImage; // Tạo biến con trỏ kiểu uint8_t trỏ đến bộ đệm ảnh
  UWORD Imagesize = ((OLED_2IN42_WIDTH%8==0)? (OLED_2IN42_WIDTH/8): (OLED_2IN42_WIDTH/8+1)) * OLED_2IN42_HEIGHT; // tính toán kích thước byte cần cung cấp
  if((BlackImage = (UBYTE *)malloc(Imagesize)) == NULL) {                                                     //cấp phát bộ nhớ động số byte
